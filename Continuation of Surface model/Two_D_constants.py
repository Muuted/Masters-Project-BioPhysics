...
def Two_D_paths(folder_names=""):
    """------ paths ---------"""
    save_path =  "2D sim results\\" + "RK test\\with Euler test 3\\"#"Data for thesis\\new test for N=40\\"#Data simulation\\"#"Verification\\"
    if folder_names == "":
...
def Two_D_Constants_stationary_state(
    ...
    #m = 1e-6 # grams  :   Mass of each chain link
    T = 1e-10 #1e-7#1e-7#1e-7#1e-8 #5e-7 #1e-6 #3e-7# 0.3e-6# 20e-7 #10 #5.45#s  : total time simulated seconds
