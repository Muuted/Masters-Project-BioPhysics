...
def Two_D_paths(folder_names=""):
    """------ paths ---------"""
    save_path =  "2D sim results\\" + "Rolling test new old\\"#"Data for thesis\\new test for N=40\\"#Data simulation\\"#"Verification\\"
    if folder_names == "":
        save_path = save_path +  "Euler2 c0=c0 tau=0\\"
    else:
...
def Two_D_Constants_stationary_state(
    ...
    const_index = 0
    if N == "":
        N = 40#25#20 #60#20#80 #int(L/ds) # 99 + 1 # Number of chain links
    #m = 1e-6 # grams  :   Mass of each chain link
    T = 5e-7 #1e-7#1e-7#1e-7#1e-8 #5e-7 #1e-6 #3e-7# 0.3e-6# 20e-7 #10 #5.45#s  : total time simulated seconds
    dt = 1e-11#1.25e-13#0.125e-12 #1e-11 #5e-11 #s time step. 
