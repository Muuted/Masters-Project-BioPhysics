...
def Two_D_paths(folder_names=""):
    """------ paths ---------"""
    save_path =  "2D sim results\\" + "Data for thesis\\test\\"#Data simulation\\"#"Verification\\"
    if folder_names == "":
...
def Two_D_Constants_stationary_state(
    ...
    """------ constants ---------"""
    if N == "":
        N = 30#40#20#20 #60#20#80 #int(L/ds) # 99 + 1 # Number of chain links
    #m = 1e-6 # grams  :   Mass of each chain link
    T = 0.5e-7 #1e-7#1e-7#1e-7#1e-8 #5e-7 #1e-6 #3e-7# 0.3e-6# 20e-7 #10 #5.45#s  : total time simulated seconds
