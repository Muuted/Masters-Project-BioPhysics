...
NPoints=5000; % number of points when evaluating solution with deval
...
p=1;
k=2;
[SDp,Yp,SD,Y,~] = PerturbShapeWavenumber(Result(k,p).ShapeSolution,k,NPoints,b,wnmin,phmin);
...
p=1; % pick one solution to perturb from all solutions in Results
k=2; %
NPoints=10000; % number of points when evaluating solution with deval
Npert=50; % vector size of ph and l
b=0.05; % amplitude of perturbations (kept constant)
wn1=0.01; % min wavenumber
wn2=2; % max wavenumber
ph1=0; % min phase
ph2=2*pi; % max phase
wn=linspace(wn1,wn2,Npert); % wavenumber vector
...
for u=1:Npert
    for v=1:Npert
        [SDp,Yp,SD,Y,~] = PerturbShapeWavenumber(Result(k,p).ShapeSolution,k,NPoints,b,wn(u),ph(v));
        [SAp] = Shape2Action(Yp,SDp,tauD,sigmaD,alpha_target); % 
        [SA] = Shape2Action(Y,SD,tauD,sigmaD,alpha_target);
...
wnmin=wn(r); % wavelength of perturbation with minimum delta S
phmin=ph(c); % phase of perturbation with minimum delta S
[SDp,Yp,SD,Y,alpha_p] = PerturbShapeWavenumber(Result(k,p).ShapeSolution,k,NPoints,b,wnmin,phmin); % evaluate solution with minium delta S
%
% %
...
colorbar;
hold on
plot(phmin,wnmin,'o','MarkerFaceColor','w','MarkerEdgeColor','k')
%
%% Plot Y and Yp
%
figure()
