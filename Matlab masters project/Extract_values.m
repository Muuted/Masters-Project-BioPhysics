...
function [sigma,tau,psi_L]= Extract_values(xmin,xmax,ymin,ymax,file)
                    ...
                    for p=1:C % loop over psi2zero values
                        if ~isempty(MS(n,m).Result(k,p).ShapeSolution) % proceed if ShapeSolution is not empty                       
                            ...
                            elseif MS(n,m).Result(k,p).ShapeSolution.ye(2,k)>psi1_cut
                                ...
                                sigma_list_Flat{n}(ah) = MS(n,m).Result(k,p).sigmaD;
                                tau_list_Flat{n}(ah) = MS(n,m).Result(k,p).tauD;
                                psi_L_list_Flat{n}(ah) = MS(n,m).Result(k,p).psi2;
