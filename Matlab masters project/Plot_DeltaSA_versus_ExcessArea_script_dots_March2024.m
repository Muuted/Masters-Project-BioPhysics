                ...
                for p=1:C % loop over psi2zero values
                    if ~isempty(MS(n,m).Result(k,p).ShapeSolution) % proceed if ShapeSolution is not empty
                        ...
                        if MS(n,m).Result(k,p).ShapeSolution.ye(2,k)<psi1_cut
                            hpoint(n)=plot(MS(n,m).Result(k,p).ExcessArea,MS(n,m).Result(k,p).DeltaSA,'o','MarkerSize',4,'MarkerEdgeColor',plot_color(n,:),'MarkerFaceColor','none','LineWidth',2); % plot (AreaD,DeltaSA) endpoint curvature above psi1_cut
                            hold on;
                        elseif MS(n,m).Result(k,p).ShapeSolution.ye(2,k)>psi1_cut
                            hpoint(n)=plot(MS(n,m).Result(k,p).ExcessArea,MS(n,m).Result(k,p).DeltaSA,'o','MarkerSize',4,'MarkerEdgeColor',plot_color(n,:),'MarkerFaceColor',plot_color(n,:)); % plot (AreaD,DeltaSA) endpoint curvature below psi1_cut
                            hold on;
...
title('Neck energy versus area (constant $\tilde{\tau}$')
