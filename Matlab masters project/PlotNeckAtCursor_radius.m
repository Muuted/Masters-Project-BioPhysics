function output_txt = PlotNeckAtCursor_radius(~,event_obj,MS,f2)
    ...
    for m=1:N_sigmaD % loop sigmaD
        ...
        for k=1:R % loop over crossings t=tau
            for p=1:C % loop over psi2zero values
                 if isequal(MS(n,m).Result(k,p).ExcessArea,pos(1)) && isequal(min(MS(n,m).Result(k,p).Y(1,:)),pos(2))
